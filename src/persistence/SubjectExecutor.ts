...
export class SubjectExecutor {
    ...
    protected broadcastAfterEventsForAll(): BroadcasterResult {
        ...
        if (this.updateSubjects.length)
            this.updateSubjects.forEach(subject => this.queryRunner.broadcaster.broadcastAfterUpdateEvent(result, subject.metadata, subject.entity!, subject.databaseEntity, subject.diffColumns, subject.diffRelations,subject.identifier));
        if (this.removeSubjects.length)
